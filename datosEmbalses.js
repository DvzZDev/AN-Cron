        ...
        for (let j = 0; j < cuenca_rows.length; j++) {
          ...
          if (cuenca_columns.length >= 3) {
            ...
            try {
              ...
              if (divs.length === 5) {
                console.log(`Encontradas 5 divs para ${embalse}, usando la segunda`)
                targetDiv = $(divs[1])
              } else if (divs.length === 4) {
                console.log(`Encontradas 4 divs para ${embalse}, usando la primera`)
                targetDiv = $(divs[0])
              } else {
                console.log(
                  `Se encontraron ${divs.length} divs con la clase 'SeccionCentral_Caja' en el embalse ${embalse}, no se cumple la condición.`
                )
                continue
              }

              const fila_seccion_divs = targetDiv.find("div.FilaSeccion")
